'''
File: tremor_analysis_cincinatti.py
...
- This program takes the raw accelerometer data for of tremor (from CINCINATTI center) in patients who had recordings conducted (both time series and frequency-power spectrum data) and analyzes them to produce metrics that can be compared across tremor groups (i.e. PD, ET, DT and controls)
- This program uses a metadata file containing the list of all patients studied (including non-identifying of initials) and stores this information
...
- The final output file containing all analysis features for each patient is used as the testing data set features for machine learning algorithms trained on seperate training data set (see README file for mor details)
'''
...
PATH = "/Users/rambalachandar/Desktop/University of Toronto/Fasano Lab/Cincinatti Data"
os.chdir(PATH)

tremor_types = ["bat","kin","out","rest"]

metadata = open('data_metadata_cincinatti.csv')
...
def num_harmonics(peaks):
    #info in each peak: [p1U,p2U,pmaxU,powmaxU,pIntegU]
...
def find_zerocross(t,sig_ff):
    ...
    #cur = [0.0,0.0,0.0] #current point
    #prev = [0.0,0.0,0.0] #prev point
...
#for butterworth signal processing
...
#Analyze time-series to calculated TSI (Tremor Stability Index)
def timeseries_analysis(file, new_path, display_tremor,setFreqRange):
    ...
    while line != "":
        ...
        u_cur = math.sqrt(x_cur**2 + y_cur**2 + z_cur**2)
        t.append(t_cur)
        x.append(x_cur)
        y.append(y_cur)
        z.append(z_cur)
    ...
    #do PCA analysis on all_axes ot find dominant axis
...
###########   Main Program: begins parsing files   #########
for ln in metadata:
    line = ln.strip("\n").split(",")
